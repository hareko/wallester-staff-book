...
class Database {
   ...
   */ {
    ...
    if ($this->dbp->prt == '') {
      if ($this->dbp->srv == 'mysql') {
        $this->dbp->prt = '3306';
      } else if ($this->dbp->srv == 'pgsql') {
        $this->dbp->prt = '5432';
      }
    }
    if (¤::IsLocal()) {
   ...
   */ {
    ...
    foreach ($flds as $fld) {
      ...
      if (($this->dbp->srv == 'mysql' && $fld['TABLE_SCHEMA'] == $this->dbn) || ($this->dbp->srv == 'pgsql' && $fld['TABLE_CATALOG'] == $this->dbn)) {
...
//        $cc = $this->Aliases($columns, $options['_ali']);
    ...
    if (isset($options['lmt'])) {
      if ($this->dbp->srv == 'pgsql') {
        $a = explode(',', $options['lmt']);
        $q .= " OFFSET $a[0]";
        if (!empty($a[1])) {
          $q .= " LIMIT $a[1]";
        }
    ...
    if (isset($options['lck']) && $options['lck'] == 'R') {
      $q .= $this->dbp->srv == 'pgsql' ? ' FOR SHARE' : ' LOCK IN SHARE MODE';
    } else if (isset($options['lck']) && $options['lck'] == 'W') {
      $q .= ' FOR UPDATE';
    }
    $dbh = $this->dbp->lnk;
