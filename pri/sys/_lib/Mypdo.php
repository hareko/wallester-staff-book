...
class Mypdo implements IData {
  ...
  //      psw -- password
    ...
    if ($this->dbDriver == 'mysql') {
      $dns = "$this->dbDriver:host=$hst;dbname=$nme" . ';charset=' . ¤::CharSet(true);
      $a[PDO::ATTR_AUTOCOMMIT] = true;
      $a[PDO::MYSQL_ATTR_USE_BUFFERED_QUERY] = false;
    }else if ($this->dbDriver == 'pgsql') {
      $dns = "$this->dbDriver:host=$hst;dbname=$nme";
//      $dns = 'pgsql:host=localhost;port=5432;dbname=testdb;user=bruce;password=mypass';
    } else if ($this->dbDriver == 'sqlite') {
      $dns = "$this->dbDriver:$nme";
    } else {
